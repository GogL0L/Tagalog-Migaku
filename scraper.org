...
Binding session to '/home/john/.nuget/packages/fsharp.json/0.4.0/lib/netstandard2.0/FSharp.Json.dll'...
[
  {
    "wheel_number": 4,
    "steering_wheel": true
...
: val getWebPageURL : char:char -> number:int -> string
...
*** Scrape site
#+begin_src fsharp :results output :session
let scrapeSite char -> int ->  MigakuDictEntry * MigakuConjEntry =
    fun charID intID ->
